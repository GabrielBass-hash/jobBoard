...
const UserDash = () => {
    ...
    const deleteUserByID = async (e, id) => {
        e.preventDefault();
        try{
            console.log("ATTENTE:", setUsers.peopleId);
            await axios.delete(`${Config.ApiBackUrl}/people/del/${setUsers.peopleId}`, {
                data: {
                    peopleId: setUsers.peopleId
                }
            });
            toast.success("Super 🎉 ! Tu la suprrimer!");
            getAllUsers();
        }catch (error){
            console.log(error)
            toast.error( "Oups.. 😢, on peut pas le supprimer...");
        ...
        <div className="flex flex-col w-full">
            {users.map((user) => (
                <div
                    ...
                    <div key={user.peopleId} className="w-2/3 flex items-center justify-between max-md:flex-wrap ">
                    ...
                    <div className="flex items-center max-md:mt-4">
                        <button onClick={() => deleteUserByID(user.peopleId)}
                                className="ml-4  text-sm px-2 text-white py-1 bg-red-el custom-radius">Supprimer
                        </button>
                    </div>
